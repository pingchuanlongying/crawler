'''
<システム名>
特定サイトクローリングプログラム「MAYORI」

<更新履歴>
2023/12/11  Ver.2.00リリース。名称を「MAYORI」とし、特定サイト用として再スタート。
2019/11/02  Ver.1.00リリース。不動産売買ポータルサイトのクローリング用としてスタート。
...
print('   |                                                                       |')
print('   |                                                                       |')
print('   +                     Portal Site Crawling System                       +')
print('   |                                                                       |')
print('   |                        M A Y O R I Ver.2.00                           |')
print('   |                                                                       |')
print('   +                                                                       +')
print('   |                                                                       |')
print('   |                        Last update: 2023.12.10                        |')
print('   |                                                                       |')
#time.sleep(0.05)
print('   +                      Made by Pingchuan Longying                       +')
print('   |                    pingchuan.longying@gmail.com                       |')
print('   |                                                                       |')
print('   +                                                                       +')
print('   |                                                                       |')
print('   |                                                                       |')
...
print('Initializing MUSUCA Ver.1.20 System...')
#time.sleep(0.9)
print('Please Wait...')
time.sleep(1.1)
...
ut = time.time()                                                                    # 変数utへ時間を格納
ftime = str(ut)                                                                     # ファイル名出力用のエポック数の取得
...
print('\n   +-----+-----+-----+-----+-----+-----+-----+-----+-----+-----+-----+-----+\n')
print('Now Scraping at ' + kyad + '/pp2_3_4_5_7/' + area + '/n-115/...')
print('   + - - - - - - - - - - - + - - - - - - - - - - - + - - - - - - - - - - - +')

# html = urlopen(kyad + '/pp2_4_5_7/' + area + '/n-124/')                           # あとで復活させる行
html = urlopen('https://www.kenbiya.com/pp2_4_5_7/s/n-124/')                        # あとで消す行
thi = random.uniform(3,5)
time.sleep(thi)
bs = BeautifulSoup(html, 'html.parser')
for link in bs.find_all('a', href=re.compile('^(/pp)(?=.*/re_1).*$')):              # 物件ページへのリンクを正規表現で探索する。
    time.sleep(2)
    if 'href' in link.attrs:
        time.sleep(2)
        html = urlopen(kyad + link.attrs['href'])
        bs = BeautifulSoup(html, 'html.parser')
        copn = bs.find('h4',text=re.compile("株式会社|有限会社|合同会社"))            # 「会社名」の抽出。find_allではなくfindとすることで最初に出現する会社名をスクレイプする。
        licn = bs.find_all('p',"license")                                           # 「免許番号」の抽出。
        zipc = bs.find_all('p',text=re.compile("〒"))                               # 「所在地」の抽出。
        addc = bs.find_all('p',text=re.compile("東京都|神奈川県|千葉県|福岡県|大分県|佐賀県|長崎県|鹿児島県|熊本県|宮崎県|沖縄県"))                        # 「所在地」の抽出。
...
xxx = input('Exit Crawing System (y|n)? -> ')
# while (xxx != 'y' and xxx != 'n'):
#    print('Your input carator is incorrect.')
#    print('Please Input y or n.')
if xxx == 'y':
    print('Crawing System Terminated.')
    os.system('cls')
elif xxx == 'n':
